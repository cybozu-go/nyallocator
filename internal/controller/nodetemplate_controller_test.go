	...
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			...
			nodeTemplateReconciler := &NodeTemplateReconciler{
				...
				Scheme:             scheme,
				RequeueRateLimiter: workqueue.NewTypedItemExponentialFailureRateLimiter[ctrl.Request](1*time.Second, 1*time.Second),
			}
		...
		It("should remove reference label from nodes when NodeTemplate is deleted", func() {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(string(body)).NotTo(ContainSubstring("nyallocator_spare_nodes{nodetemplate=\"test\"}"))
